...
<div class="card" style="max-width:600px"><div class="card-body p-4">
  <form method="POST" action="<?= url('requirements/' . $req['id'] . '/edit') ?>">
    <?= csrfField() ?>
    <div class="mb-3"><label class="form-label">Projekt</label>
      <select name="project_id" class="form-select">
        <option value="">Kein Projekt</option>
        <?php foreach ($projects as $p): ?>
        <option value="<?= $p['id'] ?>" <?= $req['project_id'] == $p['id'] ? 'selected' : '' ?>><?= e($p['name']) ?></option>
        <?php endforeach; ?>
      </select></div>
    <div class="mb-3"><label class="form-label">Name</label>
      <input type="text" name="name" class="form-control" value="<?= e($req['name']) ?>" required></div>
    <div class="mb-3"><label class="form-label">Description</label>
      <textarea name="description" class="form-control" rows="4"><?= e($req['description']) ?></textarea></div>
    <div class="row g-3 mb-3">
      ...
      <div class="col-md-6"><label class="form-label">Priority</label>
        <select name="priority" class="form-select">
          ...
          <option value="<?= $v ?>" <?= $req['priority']===$v?'selected':'' ?>><?= $l ?></option>
          <?php endforeach; ?>
        </select></div>
    </div>
